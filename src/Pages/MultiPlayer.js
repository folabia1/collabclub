...
export function MultiPlayer() {
  ...
  async function handleSubmit(event) {
    ...
    const trackResponse = await checkSongForArtists(songGuessData);
    console.log(
      `"${event.target[0].value}" Accepted: ${trackResponse.data.trackFound}`
    );
  ...
  return !/^\d{3}$/.test(roomParams) ? (
    <p>
      Oops! <strong>Room "{roomParams}"</strong> is not a guest room. Guest
      rooms only for now.
    </p>
  ) : !user ? (
    <p>
      Must be logged in to enter a room. You can continue as a guest if you'd
      like.
    </p>
  ) : (
    <div className="roomContainer">
      ...
      <p>Role: {userRole}</p>
      {user ? (
        <FeaturePathFinder onSubmit={handleSubmit} featurePath={featurePath} />
      ) : (
        <ChooseGuestUsername />
      )}
